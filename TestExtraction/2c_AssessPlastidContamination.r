...
# setwd('C:/Users/jgwall/Desktop/000_UpdateNAPB_DC/CeceliaMicrobiome/2_Analysis_additions')
# args=parser$parse_args(c("-i", "2b_tmp.Rdata", "-o", "2c_tmp"))
...
ggplot(counts, mapping=aes(x=sample, y=amount, fill=source)) + 
...
ggplot(got_targets, mapping=aes(x=treatment, y=organs)) + 
...
# ##########
pretty_data = got_targets

pretty_data$treatment = sapply(as.character(pretty_data$treatment), switch,
...
ggplot(pretty_data, mapping=aes(x=treatment, y=organs)) + 
...
# Plot by sample type
ggplot(pretty_data, mapping=aes(x=sample.type, y=organs)) + 
  geom_violin(scale="width", fill='lightblue', size=0.25) +                                          # Violin plots
  geom_jitter() +                                                       # Scatter points of actual data (jittered to be easier to see)
  labs(x="Sample Type", y="Fraction Organellar Reads") +
  theme_bw() + 
  theme(
    axis.title = element_text(size=11, face="bold"),
    axis.text = element_text(size=6, face="bold")
  )
ggsave(paste(args$outprefix, ".violin_plot.publication.by_sample.png", sep=""), 
       width=78, height = 78, units="mm", dpi=300)
