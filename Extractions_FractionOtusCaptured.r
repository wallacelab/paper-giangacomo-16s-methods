...
myplot = ggplot(shared, mapping=aes(x=treatment, y=otu_count)) +
  ...
  labs(alpha="OTU category", fill="Extraction Method") +
  ylab(ylabel) +
  xlab(NULL) + 
...
ggsave(myplot, filename = paste(args$outprefix, ".png", sep=""), width=5, height=5)
ggsave(myplot, filename = paste(args$outprefix, ".svg", sep=""), width=5, height=5)
