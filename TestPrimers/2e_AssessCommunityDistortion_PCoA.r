...
# Subset to all nonplant samples
nonplant_types = c("defined-community", "soil-clay", "soil-flowerbed")
nonplant_samples = rownames(metadata)[metadata$sample.type %in% nonplant_types]
nonplants = subMDS(distances, metadata, nonplant_samples)

# Plot by sample type
plot.mds(nonplants, paste(args$outprefix, ".all_by_sample_type.png", sep=""), color=sample.type)
plot.mds(nonplants, paste(args$outprefix, ".all_by_treatment.png", sep=""), color=treatment_std)

# Loop over sample types and plot individualls
for(mytype in nonplant_types){
